...
- **Regex là bản rút gọn, không phải bản dịch 1-1.** Ô search trong game có **giới hạn ký tự** (mặc định ~50, game sẽ nâng lên 250 trong patch tương lai), nên công cụ chỉ nhồi được vài affix ưu tiên đầu mỗi ô. Phần bị cắt sẽ ghi rõ ở mục "Đã bỏ".
- **Copy NGUYÊN cả dấu ngoặc kép `"…"` nếu có.** Token nhiều khi chứa dấu cách (vd `imum L` = "maximum Life"); trong game dấu cách bị hiểu là tách nhiều điều kiện, nên phải bọc ngoặc kép để dấu cách thành ký tự thật — `|` vẫn là "hoặc" bên trong ngoặc. Công cụ tự bọc giúp bạn rồi, chỉ cần copy nguyên.
