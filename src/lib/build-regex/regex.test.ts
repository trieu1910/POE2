...
describe('buildIngameRegex — lọc gắt (loại đồ + VÀ 2 stat, đặc trưng trước)', () => {
  it('khóa loại đồ (từ cuối tên base) + VÀ 2 stat chính', () => {
    ...
    expect(r!.regex).toBe('"Bow" "Physic" "Hit Cha"');
  ...
  it('chỉ 1 stat → loại đồ + 1 stat', () => {
    const r = buildIngameRegex(
      slot({ base: 'Dragonscale Boots', affixes: [affix('increased movement speed', 'increased Movement Speed')] }),
    );
    expect(r!.regex).toBe('"Boots" "vement"');
  });

  it('gộp 2 dòng cùng stat thành 1', () => {
    const r = buildIngameRegex(
      slot({
        base: 'Twin Bow',
        affixes: [
  ...
  it('token fallback bị đánh cờ ⚠', () => {
    const r = buildIngameRegex(slot({ base: 'Foo Ring', affixes: [affix('some weird stat', 'some weird stat')] }));
