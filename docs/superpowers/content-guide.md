...
| Path of Building 2 (PoB2) | https://github.com/PathOfBuildingCommunity/PathOfBuilding | Tính toán build |
...
- [ ] Đúng thứ tự block: `:::tip` → nội dung (có phần "làm thế này") → `<Hop variant="thuatngu">` → `<Hop variant="nguon">` → (nếu có) `:::note[📊 Số liệu sống]` **NẰM SAU** `</Hop>` của nguồn, KHÔNG lồng bên trong → `<Hop variant="mauchot">` (3–5 bullet).
