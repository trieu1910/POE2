...
- [ ] **Link NỘI BỘ trong thân bài/aside phải prefix base `/POE2/`** (vd: `[Phòng thủ](/POE2/lo-trinh/chuong-2-co-che-cot-loi/phong-thu/)`). Starlight KHÔNG tự thêm base cho link markdown trong nội dung — thiếu `/POE2/` là build sẽ báo invalid link. *(Riêng link trong `sidebar` của `astro.config.mjs` thì GIỮ NGUYÊN `/lo-trinh/...` — Starlight tự thêm base cho sidebar.)*
